    ...
    <body class="main-layout">
        ...
        <div class="container mt-4">
                ...
                <table class="table table-striped" id="justificacionesTable">
                    ...
                    <tbody>
                        ...
                        try {
                            ...
                            // Inside your while loop for displaying table rows
                            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                ...
                                echo "<tr>";
                                echo "<td>" . (isset($row['JustificacionID']) ? htmlspecialchars($row['JustificacionID']) : '') . "</td>";
                                echo "<td>" . (isset($row['dni_estudiante']) ? htmlspecialchars($row['dni_estudiante']) : '') . "</td>";
                                echo "<td>" . (isset($row['nombre_estudiante']) ? htmlspecialchars($row['nombre_estudiante']) : '') . "</td>";
                                echo "<td>" . (isset($row['Fecha_Justificacion']) ? htmlspecialchars($row['Fecha_Justificacion']) : '') . "</td>";
                                echo "<td>" . (isset($row['tipo_justificacion']) ? htmlspecialchars($row['tipo_justificacion']) : '') . "</td>";
                                echo "<td><span class='badge " . $badgeClass . "'>" . (isset($row['Estado']) ? htmlspecialchars($row['Estado']) : '') . "</span></td>";
                                echo "<td>" .
                                    (isset($row['Fecha_Inicio']) ? htmlspecialchars($row['Fecha_Inicio']) : '') .
                                    " al " .
                                    (isset($row['Fecha_Fin']) ? htmlspecialchars($row['Fecha_Fin']) : '') .
                                    "</td>";
            ...
            <button type='button' class='btn btn-danger btn-sm' 
                    onclick='confirmarEliminacion(" . (isset($row['JustificacionID']) ? $row['JustificacionID'] : '') . ")'><i class='bi bi-trash'></i> Eliminar</button>
          </td>";
                                echo "</tr>";
                            }
                        } catch (PDOException $e) {
        ...
        <script>
            ...
            $(document).ready(function() {
                $('#justificacionesTable').DataTable({
                    ...
                    order: [
                        [3, 'desc']
                    ], // Ordenar por fecha de justificación descendente
                    columnDefs: [{
